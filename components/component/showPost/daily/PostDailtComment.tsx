...
const PostDailComment = observer(() => {
  useEffect(() => {
    const getPost = async () => {
  ...
  return (
    <div>
      {getPostStore.posts
        .filter((post) => {
          return new Date(post.date).toLocaleDateString("en-GB") === new Date(setterStore.calendarDay).toLocaleDateString("en-GB");
        })
...
export default PostDailComment;
